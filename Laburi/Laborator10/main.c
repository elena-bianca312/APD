...
int* get_dst(int rank, int numProcs, int leader) {
	...
	*/
	for(int j = 0; j < numProcs; j++){
		if(v[j] == rank){
			MPI_Send(v, numProcs, MPI_INT, j, 0, MPI_COMM_WORLD);
		}
	}
	...
	for (int k = 0; k < CONVERGENCE_COEF; k++) {
		...
		*/
		for(int i = 0; i < num_neigh; i++){
			...
			MPI_Recv(&q, 1, MPI_INT, MPI_ANY_SOURCE, 1, MPI_COMM_WORLD, NULL);
			leader = fmax(q, leader);
		}
	...
	for (int k = 0; k < CONVERGENCE_COEF; k++) {
		...
		*/
		for(int i = 0; i < num_neigh; i++){
			...
			MPI_Recv(&recvd, 1, MPI_DOUBLE, neigh[i], 0, MPI_COMM_WORLD, NULL);
			val = (val + recvd) / 2;
		}		
	}		
	...
	for(int i = 0; i < nProcesses; i++){
		if(rank == parents[i]){
			for(int j = 0; j < nProcesses; j++){
				MPI_Recv(vTopology[j], nProcesses, MPI_INT, i, 0, MPI_COMM_WORLD, NULL);
