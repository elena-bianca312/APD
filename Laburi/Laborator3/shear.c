...
void *thread_function(void *arg) {
	...
	for (int k = 0; k < log(N) + 1; k++) {
		...
		for (int i = start; i < end; i++) {
			// Liniile impare descrescator
			if (i % 2) {
				qsort(M[i], L, sizeof(int), cmpdesc);
			// Liniile pare crescator
			} else {
				qsort(M[i], L, sizeof(int), cmp);
			}
		}

		// Asteptam sa termine toate thread-urile pentru linii
		pthread_barrier_wait(&barrier);

		// Sortam coloanele crescator
		for (int i = start; i < end; i++) {

			// Salvam coloana intr-un vector auxiliar
			for (int j = 0; j < L; j++) {
				aux[j] = M[j][i];
			}

			// Sortam coloana crescator
		...
		// Asteptam sa termine toate thread-urile pentru coloane
		pthread_barrier_wait(&barrier);
	}
