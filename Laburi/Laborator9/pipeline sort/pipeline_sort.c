...
int main(int argc, char * argv[]) {
	...
	if(rank==0) { // This code is run by a single process
		...
		MPI_Bcast(&initialValue, 1, MPI_INT, 0, MPI_COMM_WORLD);
		...
		for (int i = 1; i < nProcesses; i++) {
			MPI_Recv(&v[i - 1], 1, MPI_INT, i, 1, MPI_COMM_WORLD, &status);
		}
		...
		for (int i = 0; i < nProcesses - rank; i++) {
			int recValue;
			MPI_Recv(&recValue, 1, MPI_INT, rank - 1, 1, MPI_COMM_WORLD, &status);
			if (initialValue == -1) {
				initialValue = recValue;
			}
			else {
				if (initialValue <= recValue) {
					MPI_Send(&recValue, 1, MPI_INT, rank + 1, 1, MPI_COMM_WORLD);
				}
				else {
